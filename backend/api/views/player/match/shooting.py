...
class PlayerMatchShootingView(BaseStatsBombView):
    ...
    def _process_shooting_data(self, match_events, player_name):
        ...
        if player_shots.empty:
            raise ValueError(
                f'No shooting events found for player {player_name}')
    ...
    def _calculate_shooting_stats(self, shots_data):
        ...
        goals = sum(1 for shot in shots_data if shot.get(
            'shot_outcome') == 'Goal')
        on_target = sum(1 for shot in shots_data if shot.get(
            'shot_outcome') in ['Goal', 'Saved', 'Saved To Post'])
        off_target = sum(1 for shot in shots_data if shot.get(
            'shot_outcome') in ['Off T', 'Post', 'Saved Off T', 'Wayward'])
        shots_blocked = sum(1 for shot in shots_data if shot.get(
        ...
        try:
            ...
            shot_conversion = (goals / total_shots *
                               100) if total_shots > 0 else 0
