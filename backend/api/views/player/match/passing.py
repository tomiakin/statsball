...
class PlayerMatchPassingView(BaseStatsBombView):
    ...
    def _calculate_passing_stats(self, passes_data):
        ...
        completed_passes = sum(
            1 for p in passes_data if p.get('pass_outcome') is None)
        ...
        assists = sum(1 for p in passes_data if p.get(
            'pass_goal_assist') is True)
        key_passes = sum(1 for p in passes_data if p.get(
            'pass_shot_assist') is True)
