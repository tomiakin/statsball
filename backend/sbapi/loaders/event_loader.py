        ...
        for _, event in events_df.iterrows():
            try:
                ...
                if not pd.isna(event.get('playerId')):
                    try:
                        player = Player.objects.get(player_id=int(float(event['playerId'])))
                    except ValueError:
                        logger.warning(f"Invalid player ID format: {event['playerId']}")
                    except Player.DoesNotExist:
                        logger.warning(f"Player not found: {event['playerId']}")

                # Get team - with safety check
                team = match.home_team if str(event.get('h_a', '')).lower() == 'h' else match.away_team

                # Base event data with safer gets
                base_event_data = {
                    'id': int(float(event['id'])),  # Handle both string and float IDs
                    'event_id': str(event.get('eventId', '')),  # Convert to string
                    'match': match,
                    'team': team,
                    'player': player,
                    ...
                    'defensive_third': bool(event.get('defensiveThird', False)),  # Convert to bool
                    'mid_third': bool(event.get('midThird', False)),
                    'final_third': bool(event.get('finalThird', False)),
                    'type': str(event.get('type', '')),  # Convert to string
                    ...
                    'h_a': str(event.get('h_a', '')),  # Convert to string
                    'situation': event.get('situation') if not pd.isna(event.get('situation')) else None,
                    'qualifiers': event.get('qualifiers', []).tolist() if not pd.isna(event.get('qualifiers')).any() else [],
                    'satisfied_events_types': event.get('satisfiedEventsTypes', []).tolist() if not pd.isna(event.get('satisfiedEventsTypes')).any() else []
                }

                # Create the appropriate event type - with safety check
                event_type = str(event.get('type', '')).strip()
                
                if event_type == 'Pass':
                    _create_pass_event(event, base_event_data)
                elif event_type in ['Tackle', 'Interception', 'Clearance', 'BallRecovery', 'Aerial', 'Challenge']:
                    _create_defending_event(event, base_event_data)
                elif event_type in ['Save', 'SavedShot'] or any(event.get(f'keeper_{x}', False) for x in ['save', 'claim', 'punch']):
                    _create_goalkeeper_event(event, base_event_data)
                elif event_type == 'Shot' or event.get('isShot', False):
                    _create_shooting_event(event, base_event_data)
                elif event_type in ['BallTouch', 'TakeOn', 'Dispossessed', 'Foul']:
                    _create_possession_event(event, base_event_data)
                elif event_type in ['Card', 'SubstitutionOn', 'SubstitutionOff']:
                    _create_summary_event(event, base_event_data)
                else:
            ...
            if event_type == 'Pass':
                ...
                _create_goalkeeper_event(event, base_event_data)
            elif event_type == 'Shot' or event['isShot']:
                _create_shooting_event(event, base_event_data)
            elif event_type in ['BallTouch', 'TakeOn', 'Dispossessed', 'Foul']:
                _create_possession_event(event, base_event_data) 
            elif event_type in ['Card', 'SubstitutionOn', 'SubstitutionOff']:
...
def _create_pass_event(event, base_data):
    """Creates a passing event with all pass-specific fields"""
    pass_data = {
    ...
    PassEvent.objects.create(**base_data, **pass_data)

def _create_defending_event(event, base_data):
    """Creates a defending event with all defensive fields"""
    defending_data = {
    ...
    DefendingEvent.objects.create(**base_data, **defending_data)
                                    
def _create_shooting_event(event, base_data):
    """Creates a shooting event with all shot-specific fields"""
    shooting_data = {
        # Big chances
    ...
    ShootingEvent.objects.create(**base_data, **shooting_data)

def _create_goalkeeper_event(event, base_data):
    ...
    GoalkeeperEvent.objects.create(**base_data, **goalkeeper_data)

def _create_possession_event(event, base_data):
    ...
    PossessionEvent.objects.create(**base_data, **possession_data)

def _create_summary_event(event, base_data):
    ...
    SummaryEvent.objects.create(**base_data, **summary_data)