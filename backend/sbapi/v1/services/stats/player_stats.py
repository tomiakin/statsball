    ...
    def calculate_season_stats(self, competition_id, season_id, player_id):
        ...
        try:
            ...
            for match_id in match_ids:
                match_stats = self.event_aggregator.get_player_match_events(match_id, player_id)
                if match_stats['status'] == 'success':
                    # Add shooting stats
                    shooting = match_stats['stats']['shooting']
                    totals['total_shots'] += shooting.get('shots', 0)
                    totals['total_shots_on_target'] += shooting.get('shots_on_target', 0)
                    totals['total_goals'] += shooting.get('goals', 0)
                    totals['total_big_chances'] += shooting.get('big_chances', 0)
                    
                    # Add passing stats
                    passing = match_stats['stats']['passing']
                    totals['total_passes'] += passing.get('passes_attempted', 0)
                    totals['total_passes_completed'] += passing.get('passes_completed', 0)
                    ...
                    totals['total_interceptions'] += defending.get('interceptions', 0)
                    totals['total_ball_recoveries'] += defending.get('ball_recoveries', 0)
            ...
            stats = {
                ...
                **totals,
                ...
                'pass_accuracy': (
                    (totals['total_passes_completed'] / totals['total_passes'] * 100)
